...
object Bootstrap {
  ...
  /* object Modal {
    ...
    case class Props(closed: () => Callback, backdrop: String = "static",
      keyboard: Boolean = true, addStyles: Seq[StyleA] = Seq(), id: String = "", CSSClass: String = "", headerText: String = "")
      ...
      def render(P: Props, C: PropsChildren) = {
        <.div(^.id := P.id, ^.className := "modal fade", ^.role := "dialog", ^.aria.hidden := true, ^.tabIndex := -1,
          <.div(DashBoardCSS.Style.verticalAlignmentHelper)(
            <.div(^.className := "modal-dialog", if (P.id == "ConfirmIntroReq") P.addStyles.toTagMod else DashBoardCSS.Style.verticalAlignCenter)(
              <.div(^.className := "modal-content", DashBoardCSS.Style.modalBorderRadius, ^.onKeyDown ==> modalClose, ^.className := P.CSSClass,
                <.div(^.className := "modal-header", DashBoardCSS.Style.modalHeader, DashBoardCSS.Style.modalHeaderPadding,
                  <.span(<.button(^.tpe := "button", ^.className := "close", ^.onClick --> closePopup, Icon.close), <.div(DashBoardCSS.Style.modalHeaderText)(P.headerText))),
                <.div(^.className := "modal-body", DashBoardCSS.Style.modalBodyPadding, C))
            )
          ))
      }
    }
